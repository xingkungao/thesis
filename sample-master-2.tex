...
\englishauthor{}
% 导师姓名职称的英文
\englishsupervisor{Associate Professor }
% 论文作者学科与专业的英文名
...
目前现有的分布式SimRank计算主要是按照Map-Reduce编程范式的直接计算，或是基于矩阵计算的方。而对于顶点数超过百万的大图，其邻接矩阵的元素规模超过万亿。
...
对于小图（大小<10MB），重复实验1000次计算其平均值；对于更大的图，重复次数设为10000。
\subsection{算法有效性}
\begin{figure}[h]
\centering
...
下面给出基于模块度优化的图划分方法。
迭代初始化时为每个顶点分配一个不同的分块ID，考虑把某个顶点$u$由分块$a$重新放入到某个新的分块$b$后，整个图$G$的模量会发生变化，
...
如果单纯考虑两者塌缩过程的区别，显然理论上lp\string_MLP算法中基于标签传播LP算法的单次迭代过程
