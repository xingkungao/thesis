...
然而想要枚举出任意长度（包括正无穷）的随机游走显然是不可行的。在现实应用中，我们考虑长度为$l < 10$的游走得到的数值可以满足一般应用。
...
这是因为我们的算法中使用了概率筛子的缘故。对于同一个概率阈值$\delta$，图的平均顶点度数越大，则小概率游走越多， 相应地，被剔除的小概率游走越多，
所以对最终精度的影响越大。 
...
\begin{aligned}
 ...
 = & \frac{w_{u,b} - w_{u,a}}{2W} + \frac{-{d_u}^2 +2d(S_a-S_b)}{2W^2} 
\end{aligned}
\end{equation}
	  ...
	  \For{$i=1$ to $\beta$}
		...
		\For{ $v \in Nei_u$}  \Comment{Loop $u$' neighbors}
		  \If {$\sum_{t \in C_v}w_t \ge bs$} \Comment{Partitions with size smaller than $bs$}
		     \State $\Delta Q_{u,C_{u},C_{v}} \gets  \frac{w_{u,C_{v}} - w_{u,C_{u}}}{2W} + \frac{-{d_u}^2 +2d(S_{C_{u}}-S_{C_{v}})}{2W^2}$;
