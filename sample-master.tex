...
如何衡量实体之间的相似性一直是各领域研究的热点，许多研究在特定应用邻域提出了各式各样的相似度度量标准。
例如Jaccard similarity[], cosine similarity[], Dice similarity[]等等。
反应在图数据中，这些相似度的核心思想都是“如果两个顶点拥有越多共同的邻居顶点，那么它们越相似”。
显然，这些度量没能够图的整个拓扑结构。最简单的反例是，对于没有共同邻点的点对，上述度量会简单认为该点对没有丝毫的相似度。
\begin{figure}[h]
  \centering
...
SimRank【】最早被用于衡量对象与对象之间关系图中对象所处于的结构上下文之间的相似性。
SimRank的核心理念借鉴了PageRank【】，即“如果两个对象都与相似的对象相关，那么它们越相似”。
它的有效性可以由图论中的随机游走模型直观地解释，又因为它基于图这种可以描述各种关系的离散结构，
因此自提出以来，SimRank在很多不同的领域都有应用。

SimRank的计算过程拥有较高的时间和空间复杂度。
为了能够处理规模日益增长的图数据，近年来学术界提出了很多致力于提高SimRank计算效率的方法。
典型的加速技巧包括对稀疏图数据使用稀疏矩阵计算技巧【】，减少随机游走生产数量的蒙特卡洛抽样方法【】，通过线性化的代数方法消除计算递归性【】，
基于分布式平台的算法【】【delta-]等等。
这些算法面向特定场景，或多或少地提高了计算效率。
另一方面，一些致力于改进SimRank有效性的新标准也相继提出，典型的有SimFusion【】，SimRank++【】[prank]等等。
此外，一些工作研究如何将SimRank运用到特定的图数据上，例如【】研究了如何在不确定图（uncertain graph)上衡量点对的相似性。

\subsection{图划分研究现状}
图划分是一个经典的NP完全问题。国内外对其进行了广泛深入的研究。
传统的划分方法可以分为以下几种：谱方法、启发式算法、智能优化算法、多层次划分算法等。
谱方法主要针对图的二划分，其基本思想是用图矩阵的第二大特征值和特征向量来实现图的划分，它的计算开销非常大；
启发式方法基于局部的启发信息对问题进行求解，典型的代表方法有KL算法和FM算法，它们能处理的图规模非常有限；
智能方法近年来比较流行，它通过对自然界的一些行为进行仿生模拟对问题进行建模，其划分质量有一定的随机性；
多层次图划分的算法的典型代表是METIS。它通过塌缩-初始化分-恢复的迭代过程首先将图的规模降低到可以接受的范围，
再基于一定的优化思想给出该小图划分，最后将划分投影为原始图上的划分。多层次方法拓展了算法的计算能力，
被广泛地应用在各类大图的划分问题。

近年来,随着互联网的普及,网络用户数快速增加。数据显示，全球最大的社交网络Facebook目前已经有20亿用户。
随着网络中用户规模的不断扩大,与之对应的网络图动辄有数十亿个顶点和上万亿条边，
普通的计算机由于内存的限制根本无法处理,这个一些常见的图计算任务带来巨大的挑战。
在此背景下，将大规模图数据划分成多个分块散布到集群中的不同计算节点，从而充分利用分布式系统成为必然。
\begin{figure*}[h]
\centering
...
\label{fig:partition}
\end{figure*}
图划分问题的主要优化目标包括：划分结果力求均衡，即每个分块中的顶点数目大致相等；
最小化横跨不同分块的边的数量。图\ref{fig:partition}展示了对同一个图的两种划分结果。
从图中可以很直观地区分出划分质量的好坏。
对超大规模图而言，其划分过程还许满足以下要求：

1）算法需要有极好的可扩展性，这要求算法本身能够充分发觉划分过程的计算可并行性，可以通过堆砌计算节点获得更大的处理能力；
...
这给基于图计算的数据分析任务（如寻找连通分量、计算三角形和、PageRank、SimRank计算等）带来了巨大的挑战。
...
其中，METIS是经典的、久经改良的单节点图划分方法；lp\string_MLP【】也是一种多层次划分方法，但是其塌缩步骤基于标签传播（LP）算法实现；
Random指对图中顶点采取随机方式直接等概率分配分块ID。
具体运行时，上述四种算法的分块目标数被设为一样的数值$block=300$，等待每个算法完全运行结束后再基于划分结果计算边割的大小。
...
\begin{figure}[h]
  \centering
  \includegraphics[width= 0.6\textwidth]{figure/edge_cut.eps}\\
  \caption{各分块之间的边割大小}
...
\begin{figure}[h]
  \centering
  \includegraphics[width= 0.6\textwidth]{figure/conv_time.eps}\\
  \caption{算法的收敛}
   \label{fig:partitionquality}
\end{figure}
\begin{figure}[h]
  \centering
  \includegraphics[width= 0.6\textwidth]{figure/running_ti.eps}\\
  \caption{算法的运行时间}
   \label{fig:partitionquality}
\end{figure}
\begin{figure}[h]
  \centering
  \includegraphics[width= 0.6\textwidth]{figure/data_scala.eps}\\
  \caption{算法运行时间与输入图大小的变化关系}
   \label{fig:partitionquality}
\end{figure}
\begin{figure}[h]
  \centering
  \includegraphics[width= 0.6\textwidth]{figure/node_scalabity.eps}\\
  \caption{}
   \label{fig:partitionquality}
\end{figure}
\begin{figure}[h]
  \centering
  \includegraphics[width= 0.6\textwidth]{figure/block_time.eps}\\
  \caption{分块数目对计算时间的影响}
   \label{fig:partitionquality}
\end{figure}
\subsection{算法的可扩展性}
...
Delta-SimRank采用增量计算的方式，不直接更新点对之间的相似性而是计算相对于上一轮迭代的增量，最后再将之前的增量聚合起来得到最终解。
...
\end{tabular}
\end{table}
从表中可以看出，DeltaSimRank相比与SimRank在计算效率上有约几十倍的提高，这得益于其增量计算方式大大较少了迭代过程中产生的中间变量；
