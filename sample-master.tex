...
Therefore, to design an efficient distributed algorithm to compute SimRank similarity for vertex pairs in large-scale graphs
based on Spark or other systems is of great significance.

This paper addresses the above problems and our main contributions are summarized as follows:
\begin{enumerate}
...
\fi
\section{研究背景与意义}
图（graph）作为一种能够刻画实体之间关系的抽象结构，在很多领域中有广泛的应用。
例如，电子商务中商品与用户之间的交易行为，科研著作中各类论文之间的相互参考引用，
Web中各个网页之间的链接，以及社交网络中用户之间的交流传播等等行为，本质上都可以
通过图来建模。 从图中挖掘深层次的信息也成了学术界以及产业界的一个重要课题。
...
在实体解析\cite{bhattacharya2006entity}领域，人们关心如何从复杂网络中推测出不同角色是否在真实世界代表着同一个真实实体。
可以看出，现实生活中对相似度的使用无处不在。

为了准确度量出“相似性”这一概念，学者们提出了各式各样的相似度指标。
其中，SimRank\cite{jeh2002simrank}是近年来比较流行的一种度量图中顶点相似性的模型。
与传统方法不同的是，它考虑了图的拓扑结构，其基本思想与著名的网页排序算法PageRank\cite{page1999pagerank}有相似之处：
如果两个顶点的邻居顶点非常相似，那么这两个顶点也相似。
SimRank相似度可以通过随机游走模型解释，具有坚实的理论基础，近年来在很多领域中得到了广泛的应用。
...
社交网络用户数量呈现出爆发性地增长，电子商务正在日益普及，万维网上网络数据也不断增多，现实生活中图数据的规模越来越大。
...
基于这些平台，用户可以使用现成的编程接口专注于自己的计算任务，快速部署出自己的解决方案，
而无需关心分布式平台底层的复杂网络通信、资源分配、资源调度等问题。

目前现有的分布式SimRank计算主要是按照Map-Reduce编程范式的直接计算，或是基于矩阵计算的方法。而对于顶点数超过百万的大图，其邻接矩阵的元素规模超过万亿。
...
图\ref{fig:simrank_example}展示了一个学校两位教授ProfA和ProfB，他们的学生StudentA和StudentB的个人主页，以及学校Univ的主页之间的引用关系。
...
如图\ref{fig:spark_stack}所示，目前Spark已经成长为包含Spark SQL，Spark Streaming, MLlib, GraphX等多个子项目的完善的生态系统。
...
\begin{enumerate}
 \item[{(1)}] Driver Program。 每一个Spark应用程序都包含一个驱动程序，它执行用户的main函数，并负责在集群调度所有的并行化操作。
当应用程序刚从客户端提交上来的时候，驱动程序连接到负责整个集群资源分配分配的Cluster Manager，并向其请求集群上可用的Executor。 
\item[{(2)}] Cluster Manager。 它是负责处理集群中各种资源申请要求的一个服务。Spark支持Mesos，YARN等模式。
\item[{(3)}] Executor。 它运行在集群中每一个Worker Node上，负责本节点上所有的计算任务。Executor通常以多线程方式并行计算具体的计算任务。
...
RDD中的数据元素必须绑定类型，通常它们存放在内存中，但Spark也提供了其它的持久化选项。
RDD通过记录其创建以来的一系列操作（lineage），也就是记录RDD如何从原始数据演变成当前状态来实现容错机制。
...
转换最重要的特点是它是一种惰性操作（lazy operation），实际的变换只有在调用动作的时候才会真正发生。
...
它为图计算和图挖掘任务提供了简洁易用的接口，极大地减轻了开发人员的编程负担。
...
图\ref{fig:property_graph}描述了一个表示反映科研工作者人事关系的图，以及该图在GraphX中对应属性图的底层存储。
\begin{figure}[h]
  \centering
...
对图的内容或者结构的改变需要通过生成一个新的属性图来实现。
对属性图的所有操作，最终都会转换成其关联的VertexRDD和EdgeRDD上的相关操作。
...
这三种视图具体细节如图\ref{fig:triplet}所示。

%\subsection{GraphX编程范式}
\begin{figure}[h]
  \centering
  \includegraphics[width= 0.7\textwidth]{figure/triplet.png}\\
  \caption{GraphX提供的三种视图对比：顶点，边，三元组}
  \label{fig:triplet}
\end{figure}

GraphX通过Graph类来暴露核心的编程接口。
...
\end{lstlisting}
  \caption{Graph类包含的部分属性与方法}
   \label{fig:Graph}
\end{figure}
\section{本文主要研究工作}
分析已有的计算可以发现，传统的单机SimRank计算方法对于大规模的图数据无能无力，
...
本文的主要研究成果如下：
\begin{enumerate}
\item[{(1)}] 针对单源点SimRank相似度的计算，提出并实现了一种基于随机游走模型的分布式计算方法。
整个算法包含三个步骤：首先生成随机游走，再对游走进行匹配计算其相遇概率，
最后汇总得出相似度。
...
\item[{(2)}] 针对大规模图数据，提出一种分布式图划分方法。
该算法能够在保留图中稠密子结构的同时，对图中顶点进行尽量均匀地划分，同时最小化划分之间边的数量（或权重）。
实验结果表明该划分方法在划分质量上可以媲美经典的METIS，同时有较好的可扩展性。

\item[{(3)}] 基于上述的图划分方法，进而提出一种计算所有顶点之间相似度的分布式计算方法。
...
\item[{(4)}] 对上述计算方法，本文分别在流行的通用计算平台Spark上给出其分布式实现，并使用真实的数据集
验证算法的精度、效率以及可扩展性。
...
本文基于Spark平台给出算法的总体实现，最后通过实验验证算法各方面的性能。

第四章：分布式全点对相似度算法。 该章首先将全点对的相似度计算划分为两个层面的相似度：分块内相似度和分块间相似度。
然后分别给出了这两个相似度的具体计算方法。接着描述如何基于上述两个相似度来计算全局相似度。
本文基于Spark平台给出算法的总体实现，最后通过实验验证算法各方面的性能。

第五章：总结与展望。总结本文的主要工作，并对下一步工作做出展望。
...
相比于单点对和全点对的相似度计算，单源点SimRank相似度的计算复杂度正好居于它们二者之间，
它在实际问题中运用得最多。
这是因为对于大规模图数据尤其是类似于社交网络这样的拓扑结构无时无刻都在变化的图来说，频繁计算所有点对的相似度开销过于巨大。
另一方面，每次只计算单点对相似度又显得粒度过细。
单源点问题的另外一种常见变种是相似度的top\string-k查询问题，即如何选取与$u$最相似的$k$个其它顶点。
诸如此类的查询在各个领域有大量的实际应用。

为了方便描述，本章首先给出一些通用的符号定义。本文使用关系$(V, E)$表示一个有向图，其中 $V$是图的顶点集合，
...
图中所有顶点的平均入度（同样也是平均出度）用符号$d$表示。
顶点$u, v$间的SimRank相似度由$s(u, v)$表示，相应地，所有点对的相似度可以写作 $n\times n$的相似度矩阵$S$，并且有$s(u, v)=s_{uv}$。

根据文献\cite{jeh2002simrank},
顶点$u, v$间的SimRank相似度用数学公式表达如下：
\begin{equation}
s(u, v) = \left\{
        \begin{array}{ll}
	1 & \quad u = v  \\
...
S^{k+1}_{uv} = \left\{
	...
	\label{eq:two}
\end{equation}
文献\cite{jeh2002simrank}已经证明，$\lim_{k\to\infty}S^k_{uv} = s(u,v)$。可以看出，该算法的时空间杂度为$O(n^2)$，时间复杂度为$O(kn^2d^2)$。
...
对于SimRank问题，随机游走从某个顶点出发，顺着图$G$中的逆边游走，每一步随机走向该顶点的某一个入邻点，那么其转移概率为：
\begin{equation}
...
如果这两条随机游走在同一个顶点$x$相遇并且为它们的初次相遇，就把这两条终止在顶点$x$的游走称作“匹配游走”。
每一对匹配游走的长度就等于随机游走的步骤数。基于此定义相遇概率：
 \begin{equation}
...
然而想要枚举出任意长度（包括正无穷）的随机游走显然是不可行的。在现实应用中，考虑长度为$l < 10$的游走得到的数值可以满足一般应用。

文献\cite{li2010fast}提出了一个计算单点对SimRank相似度的算法spSimRank。算法的核心思想是两个随机冲浪者分别从顶点$u,v$出发，
每一步顺着所在顶点的入边移动，并且每经过一个顶点$t$，原来的随机游走会分裂为$|I(t)|$ 个新的游走。这样的话经过$k$移动之后，总共会产生
...
如果从这个算法出发，可以轻易得出一个通过暴力枚举求单源点SimRank相似度的方法，如算法\ref{algo:nssSimRank}所示。
...
然而经过简单分析，可以发现这种方式有以下的缺点：
\begin{enumerate}
\item[{(1)}] 随机游走的数量过大。 算法总共生成了$O(nd^k)$个游走，但其中的大多数都无法顺利相遇即匹配。
\item[{(2)}] 空间复杂度较高。 尽管Path-Tree这种数据结构已经非常高效，但是如果生成$n$个Path-Tree，空间复杂度仍然比较高。
不难想象，这些Path-Tree中一定存在很多重叠的子树结构，存储所有的Path-Tree会有较大冗余。
\item[{(3)}] 在最后的匹配过程，同一长度的任意随机游走是以暴力方法匹配的，时间复杂度较高。
\end{enumerate}
...
\begin{fact}
假设两个分别从顶点$u, v$开始的随机游走$W_u, W_v$经过$l$步后在顶点$x$相遇，那么如果从顶点$x$ 出发生成所有长度为$l$的倒序游走，
$W_u, W_v$ 的倒序游走一定也在其中。
\end{fact}
\begin{fact}
 假设$W_u=uw_1w_2\dots w_l$是一条主游走，则所有有可能与$W_u$在 $l$步相遇的从游走，只可能在$\{w_1, w_2, \dots, w_l\}$中的任意一点相遇。
...
为了解决这个问题，对$|Nei|$中的每一个顶点$v$，本文并不使用任何特别设计的数据结构单独存储从$v$的所有随机游走，
而是直接使用从$v$开始顺着逆边经过$k$步可达的一个邻域$N_{G}(v, k)$。
严格地说，$N_{G}(v, k)$是顶点$v$顺着逆边$k$步可达的顶点集合在$G$中的导出子图（induced subgraph）。
\begin{figure}[h]
    \centering
    %\subfig[fjalkf]
    ...
    \end{subfigure}
    \caption{ $v $的一个邻域及其展开得到的Path-Tree}
    \label{fig:match_walks}
\end{figure}

图\ref{fig:match_walks}中展示了$v$的一个邻域以及它所对应的Path-Tree，灰色的顶点是查询顶点$u$。
Path-Tree中从根节点到每一个其它节点之间的路径都对应一条游走。
可以看到，这些长短不一的游走的前缀子序列存在高度重合。相比于Path-Tree，$N_{G}(v, k)$自己本身就是一种更紧凑的存储随机游走的数据结构。
显然，本文没有显式地表示出每一条游走，那么在后面过程中必然要通过某种方式将它们表达出来。
本文采用$N_{G}(v, k)$这种数据结构有以下原因：
\begin{enumerate}
 \item[{(1)}] 在分布式环境中,为了生成游走计算节点需要进行消息传递来获取非本地的图结构信息，
大量的随机游走需要在网络上传送。
因而使用什么样的数据结构表示它们直接关系到计算过程中的中间传输数据量，其所带来的网络开销会极大地影响算法的最终性能。
\item[{(2)}] 因为$N_{G}(v, k)$包含了从顶点$v$出发的所有游走，那么在后续的匹配过程中，可以把邻域当做最小的匹配单位。
这意味着在分布式环境下，将对应于不同$v$的邻域$N_{G}(v, k)$以$v$为key分发到集群中（例如通过Spark的HashPartitioner），
剩下的匹配过程就可以在所有计算节点上分布式地并行计算，从而整个算法具有高并行度。
\item[{(3)}] 本文在下一节会详细叙述如何基于动态规划技巧直接在邻域$N_{G}(v, k)$上进行高效地匹配计算，而无需将随机游走从$N_{G}(v, k)$重新提取出来。
\end{enumerate}

\subsection{使用动态规划技巧加速随机游走的匹配}
当$Nei$中的每个顶点$v$收集到了从它自己开始的倒序游走$N_{G}(v, k)$之后，这些从游走需要与所有的主游走$N_{G}(u, k)$进行匹配，从而计算相遇概率。
由于代表所有主游走的邻域$N_{G}(u, k)$只有很小的空间开销($O(d^k)$)，可以把它预先广播（broadcast）到分布式集群中的每一个计算节点上。
所有的从游走$N_{G}(v, k)$按照$v$作为key被分发到不同的节点上，每个计算节点可以直接在本地进行匹配过程。
这样，算法中最耗时的匹配计算被分发到每个计算节点上进行的，使得算法具有极佳的并行性。

为了方便叙述，这里以在顶点$v$第一次相遇的随机游走为例。
具体任务是：给定主游走$N_{G}(u,k)$，在邻域$N_{G}(v, k)$中找出所有能够匹配的（主游走，从游走）对。
注意到只需要关注与主游走在$v$相遇，但是之后再也不和主游走有任何共同顶点的逆序游走。
例如，在\ref{fig:match_walks_two}中，路径$vau$和$vbe$是一对对计算$s(u,e)$有意义的匹配游走。
同理，能和$vau$匹配上的随机游走还包括$vcf$。
但是，$vad$和$vau$不是一对合法的匹配游走，因为它们首次在节点$a$而不是$v$首次相遇。实际上，
$vad$和$vau$同样对$s(d,u)$的计算做出了贡献，但是在分布式环境下，它们的匹配计算过程发生在$N_{G}(a, k)$所在的计算节点上。

基于以上观察可以发现以下事实：
\begin{fact}
如果查询顶点$u$处于某个Path-Tree的某一层里，那么那一层的其它任一满足$LCA(u, w)=v$的顶点$w$都会点对$u,w$的相似度$s(u,w)$的计算做出
大小为$c^{l}Pr(W_u)Pr(W_w)$的贡献，其中$LCA$指的是最近公共祖先(Lowest Common Ancestor)，
$W_u, W_w$是由对应Path-Tree展开的root-to-node路径。
\end{fact}
基于以上事实，本文把匹配游走这一问题看做是一个动态规划(Dynamic Programming)问题，
通过Memorization的技巧来降低匹配的时间复杂度。
对每一个$1 \leq l \leq k$，使用DFS算法在邻域$Nei_G(v,k)$搜索长度为$l$的匹配游走。
...
例如，在图\ref{fig:match_walks_two}中，能和游走$vbdu$匹配的唯一游走是$vbef$，而计算$vbef$的概率时，只需要从节点$e$开始DFS过程，
...
算法首先循环$W_l$中的每一条主游走$p$，并且知道该主游走在Path-Tree中的哪一棵子树中（第2-3行），如果$M_l$中没有记录该子树中匹配游走的概率，
就开始DFS过程（第４行）。
先检查$W_l{\prime}$中是否有记录该子树的信息，如果有的话就直接从记录的游走的终止节点开始调用DFS(第5-6行），
否则就需要从该子树的根节点开始DFS。
通过对所有的$W_l (l \leq k)$运行LevelMatch算法，所有游走的相遇概率都可以高效地计算出来。

\subsection{使用概率阈值剔除极小的概率}
很多现实的大图都是scale-free\cite{li2005towards}的，这意味这图中的极小比例的顶点会有极高的度数。
而算法的性能与顶点的度数紧密相关，因为在生成游走过程中每条游走每经过一个度数为$d$的顶点，都会在那个顶点分裂为$d$条更长的游走。
因此，本章使用一个阈值\cite{lizorkin2008accuracy}$\delta$来删除那些包含多个拥有极高度数的顶点的游走，
因为根据公式\ref{eq:seven}，这些游走的概率非常之小，对最终计算精度的影响基本可以忽略不计。
$\delta$的使用展示在算法\ref{alg:dp}的第14-15行。
...
\begin{enumerate}
 \item[{(1)}] 找出从顶点$u$出发顺着入边经过$k$步可达的邻域$Nei$，同时记录所有的主游走。然后将所有的主游走广播到集群所有计算节点上。
 \item[{(2)}] 从$Nei$中每一个顶点$v$出发，顺着出边找出其$k$步可达的邻域，这些邻域看做一个以$v$为key的key-value对，分发到集群中。
 \item[{(3)}] 将上面每一个$v$的邻域和第一步中的$Nei$进行匹配，计算相遇概率。这一过程是分布式的。
 \item[{(4)}] 聚合所有的相遇概率得出最终的相似度$s(u,*)$。
\end{enumerate}
上述四个步骤中第1个和第2个很相似，区别只是要获得邻域的顶点的数量（1 vs $|Nei|$）。
因此本章重点描述第2、 3、 4步骤在Spark平台上的实现。

首先需要将表示图的原始文本数据上传至分布式文件系统HDFS上，然后Spark读入图数据，生成对应的RDD。
...
首先就把图从按照边存储的格式变为邻接表这种格式，并存储在$graphRDD$中（第2-3行），
然后从$graphRDD$生成$nbrhdRDD$（第4行），这个RDD里面存放着所有的邻域。
...
当所有邻域全部生成完毕后，算法开始匹配主游走和从游走。具体细节如算法\ref{algo:match}所示。
...
对$nbrhdRDD$中的每个元素$(v,nbrhd)$，调用算法\ref{alg:dp}计算$nbrhd$中所有游走的相遇概率。
...
\subsection{实验数据集}
本章一共使用了5个真实的图数据来评估算法的性能。 数据集的具体细节如表\ref{tab:dataset1}。
...
在本章的实验中，直接选取$\epsilon=0.01$，因为这样的精度可以满足大多数实际应用。 
...
对于小图（文本大小小于10MB），重复实验1000次计算其平均值；对于更大的图，重复次数设为10000。
\subsection{算法有效性}
\begin{figure}[h]
\centering
\begin{subfigure}[b]{0.48\textwidth}
	\center
	\includegraphics[width=1\textwidth]{figure/conv_11p_quality.eps}
...
\end{subfigure}
\caption{计算误差随迭代次数的变化曲线比较}
\label{fig:ch1:effect}
\end{figure}
首先比较本章的算法sssSimRank和标准计算方式apSimRank的时间收敛速度。
选取的误差指标为平均误差(mean error)，即$ME = \frac{1}{n}\sum_{v \in V}{\left|s(u,v) - s^k(u,v)\right|}$，
其中$s(u,v)$为根据公式\ref{eq:two}迭代计算至完全收敛的真实相似性，$s^k(u,v)$为本章算法迭代$k$次后的相似性。
我们在两个小图上进行实验。其中p2p-gnutella08是个比
较稀疏的图，平均顶点度数$d=3.29$；而wiki-vote是一个更加稠密的图，平均顶点度数为$d=15.57$。
图\ref{fig:ch1:effect}为最终的比较结果。
从图中可以看到，二者在6次迭代以内精度都得到了收敛， 本章的算法sssSimRank有更好的收敛速度，三次迭代之后平均误差就在$10^{-4}$以内。
这是因为本文的算法“抓大放小”剔除了小概率游走，而apSimRank仍然需要继续计算它们。
另一个现象是在图\ref{fig:ch1:effec:one}中sssSimRank的平均误差与apSimRank的差距比图\ref{fig:ch1:effec:two}更小，
这是因为算法使用了概率筛子的缘故。对于同一个概率阈值$\delta$，图的平均顶点度数越大，则小概率游走越多， 相应地，被剔除的小概率游走越多，
所以对最终精度的影响越大。 
...
图\ref{fig:ch1:runtime}还表明，本章的算法在数据集eu-2005和arabic-2005减少邻域的比例比图p2p-gnutella08和ljournal-2008要大很多，
这一观察同样表明算法中的概率筛子对稠密的图能更好地提高性能。
\subsection{算法的可扩展性}
实验考察分布式环境下算法的可扩展性。 首先考察算法运行时间随着输入图的大小的变化关系，结果如图\ref{fig:ch1:data_scalable}所示。
...
图划分按划分对象归类为两类：切分边(edge cuts)和切分顶点(vertex cuts)的划分。
...
本文关注切分边的划分方式，如无特殊说明，本文下面提到的图划分都是指对图中顶点的划分。
...
\begin{enumerate}
\item[{(1)}] 算法需要有极好的可扩展性，这要求算法本身能够充分发掘划分过程的计算可并行性，可以通过堆砌计算节点获得更大的处理能力；

\item[{(2)}] 划分结果需要较好地保留原图的语义。
现实生活中的图并不是随机生成的，它们的拓扑结构往往隐含着语义（semantic）层面的信息。好的划分算法应当尽量保留图中的语义信息。
\end{enumerate}

因而设计一个好的分布式图划分方法具有重要的显示意义。
针对以上问题，本章提出一种高效的分布式图划分算法，实验结果表明，该方法具有较高的效率以及较好的可扩展性。
...
尽管上面几个流行的平台都开放了一些接口支持用户按照需求设计自己的划分方法，但都没有提供一个直接可用的、高效的分布式图划分方法。
...
第一种方式对图的邻接矩阵进行二维的划分，例如对9$\times$9的矩阵，将其均匀划分成9个3$\times3$的分块，再把这些分块分发到集群中的9个节点上去。
...
这种应用场景下图中的顶点以数据流的形式流入系统，算法无法感知图的全局拓扑结构。这类方法使用一些简单的局部启发式信息对图进行划分，因而划分质量往往不是最优。

\section{算法的基本框架}
本章从以下两方面来考虑图划分算法的优化目标：
\begin{enumerate}
 \item[{(1)}] 划分结果顶点的分配均衡。 理想情况下，每个分块中顶点的数目应该尽可能相似。 
 如果用$bs$表示分块中顶点数目的上限，那么对划分$P=(C_1, C_2, \dots, C_n)$，应该有$\max\limits_{i}|C_i| \leq bs$。
约束条件$bs$的设置可以与具体场景有关，比如集群中计算节点的内存容量必须是$bs$的一个上限。
 \item[{(2)}] 划分结果的质量。 理想情况下图划分算法应该可以把图中稠密的子图归类于一个分块中。
 但是图中稠密子图的规模天然是不均匀的，而划分过程中有负载均衡的限制，因此需要从整体考虑划分的质量。
 设$P$是图$G(V,E)$的一个划分，定义其边割(edge-cut)为
 $EC =\sum\nolimits_{e(u,v) \in E, u \in C_i, v \in C_j} w\big(e(u,v)\big)$，即所有连接不同分块的边的权重之和。
 理论上，$EC$应尽可能的小。
\end{enumerate}
本章的算法借鉴多层次划分（Multi-Level Partitioning）的思想,主要有下面三个步骤：
\begin{enumerate}
 \item[{(1)}] 首先不断地通过塌缩步骤使原图的规模越来越小。
每次进行塌缩时，先对图做效果有限的简单划分，然后将划分结果的每一个分块当做一个新的顶点，分块之间的边当做一条新的边，形成一个规模小很多的图。
\item[{(2)}] 经过多次迭代塌缩后，图的规模小到某个可以接受的阈值，再使用KL或FM等传统高质量单节点算法直接进行划分，得到初始划分的结果。
\item[{(3)}] 最后将初始化分划直接投影到原图中，这样就得到了最终的划分结果。
\end{enumerate}
...
\begin{enumerate}
 \item[{(1)}] 边的权重赋值方式使得划分塌缩后的图所的边割代价与划分原图保持一致，这样可以保证图划分质量的一致性。
 \item[{(2)}] 顶点的权重赋值方式使得划分塌缩后的图的负载均衡状况与划分原图保持一致，这样可以保证划分过程中负载均衡的一致性。
\end{enumerate}
...
本章的塌缩过程借鉴了社区发现(Community Detection)的思想。
...
\fi
本文使用了模块度\cite{newman2006modularity}(modularity)最大化的思想从$G_i$生成$P_i$。
...
用数学语言描述，设$d_v$表示顶点$v$的度数, $C_v$表示顶点$v$所在社区的ID，图的模块度定义如下：
\begin{equation}
\label{eq:mod}
...
\begin{figure}[t]
	...
	\label{fig:figure2:figure2}
    \end{subfigure}
    \caption{基于最大匹配与基于模块度的塌缩方式结果比较}
    \label{fig:modu_paritition}
\end{figure}
这个定义表明模块度是一个取值在-1与1之间的标量，并且模块度越大，其社区划分的质量越高。
按照上面的定义，可以得到如下的定理：
\theorem{根据上面的定义，实际上可以得到模块度$Q$定义的简单形式：
\begin{equation}
...
\begin{proof}
根据\ref{eq:mod}的定义，如果顶点$u, v$属于不同社区的话，那么显然有$\delta(C_u, C_v)=0$，
因此只要考虑属于同一社区的点对，即可以把公式\ref{eq:mod}改写为：
\begin{equation}
\begin{aligned}
...
下面给出基于模块度优化的图划分方法。
迭代初始化时为每个顶点分配一个不同的分块ID，考虑把某个顶点$u$由分块$a$重新放入到某个新的分块$b$后，整个图$G$的模块度会发生变化，
用符号$\Delta Q_{u,a,b}$表示。
根据模块度的定义，有：
\begin{equation}
\begin{aligned}
\label{eq:deltaQ}
 \Delta Q_{u,a,b}  = & \left[ \frac{I_b+w_{u,b}}{2W} - {\left(\frac{S_b + d_u}{2W} \right)}^2 \right] -
 \left[ \frac{I_b}{2W} - {\left(\frac{S_b}{2W} \right)}^2 \right] \\
 &+ \left[ \frac{I_a-w_{u,a}}{2W} - {\left(\frac{S_a - d_u}{2W} \right)}^2 \right] -
 \left[ \frac{I_a}{2W} - {\left(\frac{S_a}{2W} \right)}^2 \right] \\
 = & \frac{w_{u,b} - w_{u,a}}{2W} + \frac{-{d_u}^2 +2d_u(S_a-S_b)}{2W^2} 
\end{aligned}
\end{equation}
其中，$w_{u,b}$指的是顶点$u$指向分块$b$的所有的边的权重之和，$w_{u,a}$也类似。
上式表明，将某个顶点从之前的分块移除并重新放入新的分块之中时，只需考虑图中的局部信息就可以计算出模块度的变化量，
而不需要按照式\ref{eq:mod}那样暴力遍历所有边。

因为Spark平台内部RDD的计算无法按照异步(asynchronize）方式生成，
本轮迭代中所有线程的计算结果需要经过严格的同步（synchronize）才会进入下一步的迭代。
所以在第$t$轮迭代中计算维护图中顶点的相关信息只能基于第$t-1$轮迭代各顶点已有的信息。
基于这个特性，本章对图进行基于模块度优化的塌缩算法如\ref{alg:mlgp}所示。
算法包含三个参数，$\alpha, \beta, k$。
其中，$\alpha$指的是反复塌缩过程的目标划分数目，如果迭代过程中当前的划分数目大于$\alpha$，就继续迭代下去。
$\alpha$取值的具体设定与第二步中单节点图划分算法可以处理的输入图规模有关。
$\beta$指的是塌缩方法（即基于模块度进行社区检测）的迭代次数。
参数$bs$代表图划分过程中每个分块可以包含顶点数目的上限。
如果$bs$设置得较大，那么最终每个分块规模会比较小，而分块数目较多；
如果$bs$设置得较小，那么每个分块规模可能较大，而分块数目较少。
首先初始时赋予图中每个顶点一个不同的分块ID（第2行）。
...
这保证了最终分块的顶点分配均衡，即每一个分块的顶点数目都不会超过$bs$。
  ...
  \State Initialize partition $P$, which assigns each vertex a unique partition ID;
	  ...
	  \For{$i=1$ to $\beta$}
		...
		\For{ $v \in Nei_u$}  \Comment{Loop $u$' neighbors}
		  \If {$\sum_{t \in C_v}w_t \ge bs$} \Comment{Partitions with size smaller than $bs$}
		     \State $\Delta Q_{u,C_{u},C_{v}} \gets  \frac{w_{u,C_{v}} - w_{u,C_{u}}}{2W} + \frac{-{d_u}^2 +2d_u(S_{C_{u}}-S_{C_{v}})}{2W^2}$;
		     \If {$\Delta Q_{u,C_{u},C_{v}} > maxGain$}
...
本文直接使用METIS软件包提供的单节点算法求解$G_t$的划分$P_t$。
...
\fi
\section{基于Spark平台的算法实现}
本文基于Spark提供的分布式图计算框架GraphX来实现算法的逻辑。
...
都单独使用一个RDD以KV对的方式记录$V_i$与$V_{i+1}$中顶点的对应关系，其中$key$是
$V_i$中顶点的ID，$value$是该顶点在$V_i$中的分块ID，同时也是$V_{i+1}$中的某个顶点ID。
这样如果迭代了$t$次就有$t$个单独的$RDD_{1}, RDD_{2}, \dots, RDD_{t}$。
当图的规模达到可以接受的范围之后，使用collect操作将其收集到Driver Program，使用单节点算法直接对其划分。
然后再使用parallelize方法将划分结果重新变成一个元素类型为（vertexID， partitionID）的RDD数据，记其为$RDD_{t+1}$。
恢复过程的具体细节如图\ref{fig:restore}所示。
对$t+1$个$RDD$，取最后一个$RDD_{t+1}$（第1行），将它与互换过KV（即由原来的$(key,value) \rightarrow (value, key)$，第3行)的$RDD_{t-1}$做join操作得到$V_t$与$V_{t-1}$中顶点的对应关系（第4行），
...
%\footnotetext[5]{http://law.di.unimi.it/webdata/arabic-2005/}
\subsection{算法的有效性}
衡量划分质量的主要依据是划分结果的边割大小。
图 \ref{fig:partitionquality} 展示了本章的算法mod\string_MLP与其它相关算法的最终划分质量比较。
...
运行过程中不设置算法的目标分块数，试图让算法“自由”地运行下去。
...
如果单纯考虑两者塌缩过程的区别，理论上lp\string_MLP算法中基于标签传播LP算法的单次迭代过程
...
实验中，塌缩过程中的内部迭代次数都被设置为$\beta=5$，结果发现lp\string_MLP的总迭代次数超过mod\string_MLP的所需迭代次数的$20\%$左右。
...
$\beta$的值分别被设置为1,3,5,10，例如图例“1-mod-cd”表示塌缩过程中采用内部迭代深度为1的基于Modularity的Community Detection算法，
...
本文首先对原图进行划分，然后计算每个分块内部点对的SimRank相似度；接着将各个分块当做单独的顶点，求取分块之间的相似度。
...
文献\cite{cao2012delta}提出了一种基于增量变化的$DeltaSimRank$算法。该算法发现SimRank计算可以改写为一种迭代的增量计算方式，
即迭代过程中，不直接计算点对之间的相似度而是计算其相对于上一轮迭代的增量。该方法充分利用了迭代计算过程中很多点对间相似度增量为零的事实，减小了
计算过程中的数据传输量，从而提高了计算效率。

\section{算法的基本框架}
基于第三章中对图做出的划分，本章给出一个计算点对之间近似SimRank相似度的算法。
文献\cite{ilprints579}描述了一种基于图中分块结构（Block Structure）的若干性质来加速计算PageRank的方法，本章的方法直接受到了该算法的启发。
...
相比原来的SimRank的计算方法，该算法表现出了与其相媲美的有效性，同时极大地提高了计算效率。

假设图已经被划分为若干分块，那么顶点对之间的相似度可以分为两种：
\begin{enumerate}
 \item[{(1)}] 位于同一稠密分块之中的顶点对。 在上一章节中，本文已经详细介绍了将原图$G$划分为若干稠密分块的方法，每一个划分好的
 分块都被存放在集群中的某一个计算节点中。 这种情况下可以根据公式\ref{eq:two}直接使用串行计算方法计算出该点对的相似性。
 \item[{(2)}] 位于不同分块之中的顶点对。如果分块的大小充分大，那么两个顶点分别处于不同的稠密分块之中意味着较大概率地这两个顶点之间的距离会比较远。
 而回顾随机游走模型，相隔较远的两个随机漫步者通常无法在短距离内（<10)通过游走在某个顶点相遇。
 这表明这两个顶点的相似性会较大概率地在数值上表现得非常小，因此直接计算它们的近似相似度。
\end{enumerate}
...
\begin{algorithm}[h]
\captionof{algorithm}{Weighted All-pair SimRank:SimRank++}
...
\begin{definition}
 ...
 \label{eq:avg}
  ...
  \begin{equation}
   ...
   \end{equation}
   即使用$u$与其分块内所有顶点的相似度的均值来估计$u$与分块中心的相似度。
\end{definition}
...
综上，本章的算法步骤主要如下：
\begin{enumerate}
 \item[{(1)}] 首先根据上一章节描述的方法，对原图$G$做划分，得到若干个分块$P=(C_1, C_2, \dots, C_n)$；
 \item[{(2)}] 将各个分块当做一个顶点进行塌缩得到一个加权有向图$G^\prime$，在$G^\prime$运行适用于加权图的SimRank++算法，得到
 分块之间的相似度；
 \item[{(3)}] 对所有分块$C_i$直接运行串行的全点对SimRank算法，得到分块内部的顶点对相似度；
 \item[{(4)}] 使用公式\ref{eq:avg}计算属于不同分块顶点之间的相似度；
 \item[{(5)}] 最后返回全局相似度。
\end{enumerate}
\subsection{算法复杂度分析}
SimRank以及其适用于加权有向图的变种SimRank++的时间复杂度都为$O(kd^2n^2)$。
...
因此总的时间复杂度为$O(n^2)$。
如果考虑主要的计算过程，即第2步和第3步，所需的时间为$O(k{d_1}^2m^2+k{d_2}^2n^2/m)$。
若假设有$d_1=d_2=d$，则时间复杂度为$(kd^2(m^2+n^2/m))$。 
将该复杂度看做变量$m$的函数，则函数在$m=(2n)^{2/3}/2$处有极值，这对正确选择合适的$m$有指导意义。
...
所有的条目一共被分为17364个不同的类别(category)。
我们挑取其中较大的四个类别（由于wiki-topcats中类别是匿名的，暂称它们为C1-C4)，
...
在开始实验前，所有的数据集都预先上传到分布式文件系统HDFS上。
\subsection{算法的有效性}
首先验证算法的有效性。严格地说，本文所提出的相似度计算方法是一个计算近似SimRank相似度的算法。
对于同一分块之内的点对，本章算法的结果与严格的SimRank相似度基本不存在差异；
对于来自不同分块的点对，显然二者在数值上会有差异。
在wiki-topcats上的实验结果表明，它们的计算结果在数值上差异不超过正负$5\%$。
为了检验算法在实际应用中语义层面的有效性，本文使用PAM\cite{kaufman:clustering1990}，一种基于顶点相似度的经典k-medoids图聚类算法，对wiki-topcats
进行聚类得出结果作为标准结果。
分别运行SimRank算法以及本章提出的算法，取被正确分类的顶点个数与顶点总数的比值作为准确率(accuracy)。
结果表明，SimRank的准确率为0.6218，本算法的准确率为0.6221。
从结果上看，本算法的准确率基本与SimRank持平或有略微的提高，
猜测这可能是我们使用了图划分预处理过程，一定程度上把原图中的稠密子图提取了出来，从而消除了噪声。

\subsection{算法的计算效率}
实验分别比较本算法partitionSimRank与基于稀疏矩阵计算的matrixSimRank计算方法、基于传输量优化的DeltaSimRank算法各自的计算效率。
其中，基于稀疏矩阵乘法的SimRank计算过程直接根据公式\ref{eq:two}做大规模矩阵乘法；
...
\end{tabular}
\end{table}
从表中可以看出，DeltaSimRank相比于matrixSimRank在计算效率上有约几十倍的提高，这得益于其增量计算方式大大减少了迭代过程中产生的中间数据；
...
\begin{figure}[h]
  ...
  \caption{arabic-2005上数据分块数目与运行时间的关系}
...
然而对于现实生活中的拥有数百万节点的大规模图数据,其邻接矩阵的元素规模达到万亿级别($|V|^2$ ),
现有的方法无法对其进行有效计算。
...
\begin{enumerate}
 \item[{(1)}] 针对单源点SimRank相似度的计算问题，即给定一个查询节点$u$，计算它与图中其它所有点对的相似度，提出并实现了一种基于随机游走模型的分布式计算方法。
 本文还充分优化了算法中的关键步骤。该算法具有效率高、并行度好的特点。
 \item[{(2)}] 为了能够将图进行划分以方便后续处理，本文提出了一种基于模块度优化的大规模图数据的分布式划分方法。 该算法能够较好地保留图中的稠密子图，
 并且划分结果做到了顶点分配均衡。实验结果表明算法具有极好的划分质量，同时拥有较高的执行效率和良好的可扩展性。
 \item[{(3)}] 基于上述图划分结果，提出了一种基于分而治之思想的全点对相似度计算方法。 算法将相似性分为两个层次：分块之间的相似性，以及同一分块内点对的相似性。
...
\begin{enumerate}
 \item[{(1)}] 本文提出的图划分算法只考虑了最终划分结果的顶点数量的分配均衡，并没有考虑每个分块内边的数量的相对均衡。
 由于边的数量往往超过顶点的数量，因此对于一些结构特殊的图，这样的策略无法得到最佳的划分结果。
 \item[{(2)}] 由于本文单源点、全点对的相似度算法都基于Spark实现，出于对平台的一致性考虑本文提出的分布式图划分算法也基于构建于Spark之上的库GraphX。
 然而，全点对相似度算法要求图划分算法以顶点为基本划分单位，但GraphX底层实现机制中图是按边划分的。这两者的差异虽然不会对最终划分质量造成影响，
 但有可能一定程度上降低了算法的运行效率。
 \item[{(3)}] 本文提出的计算全点对复杂度的分布式计算方法计算的是近似的SimRank，虽然其有效性在真实数据集上得到了验证，
 但是本文的近似过程比较粗糙，并没有严格的理论证明。 对于一些结构特殊的图，本文提出的计算方式可能会与实际SimRank数值存在偏差。
...
``Scalable Single-Source SimRank Computation for Large Graphs,'' in \textsl{Proc. 22nd IEEE International
    ...
    \item Jie Liu, Xingkun Gao, Nianyuan Bao, Jie Tang, and Gangshan Wu,
    ``Deep Convolutional Neural Networks for Pedestrian Detection with Skip Pooling,'' in \textsl{Proc. 30th 
    International Joint Conference on Neural Networks (IJCNN) 2017}, May. 2017.
\end{publications}
