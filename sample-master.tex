...
社区，从直观上来看，是指网络中的一些密集群体，每个社区内部的顶点间的联系相对紧密，但是各个社区之间的连接相对来说却比较稀疏。
在社区检测过程中，图中局部联系紧密的子结构往往会共享同一个社区ID，
与其他图划分方法（例如最大匹配算法）相比，基于社区检测的划分方法能够更好地感知原图中的语义。
二者最大的区别在于，社区发现问题往往不会固定需要发现社区的数量，因为具体社区的个数是由输入图的内在结构决定的；
而图划分问题一般都会设定需要划分的分块个数$k$。

LP算法的思想非常简单：最初的时候我们给图中每个顶点点一个单独的社区ID，
然后我们迭代式地为每个顶点更新社区ID。
每次迭代中，每个顶点按照其邻居顶点所属社区ID出现最多的那个作为自己新的ID。
当$G$中每个节点的社区ID基本上不再变动时，算法结束。
图展示了一个典型的社区发现结果。
从图中可以直观的看出，总共有4个社区。如果直接将其当做图的一个划分$P=(C_1, C_2, C_3, C_4)$，其划分质量也是不错的。
\begin{figure}[htbp]
  \centering
...
而确定每个顶点所属社区ID这一步骤是迭代过程中的计算主题，因此有很搞的提升空间。

本文借鉴了文献【７】中模块度(modularity)的思想，试图在迭代过程中有方向性地寻找合适的社区ID。
模块度是常用的一种衡量网络中社区稳定度的方法。
简而言之，对于一个已经分配好社区的图，分配在同一社区的边占总边数的比例，与对这些边随机分配得到的概率期望，这两者的差就是其模块度。
用数学预言描述，设$d_v$表示顶点$v$的度数, $C_v$表示节点$v$所在社区的ID，图的模块度的定义如下：
\begin{equation}\label{eq:modularity}
Q = \frac{1}{2m}\sum\limits_{u, v \in V} \left[w_{u,v} - \frac{{d_u}{d_v}}{2m} \right]\delta \left(C_u, C_v\right),
\end{equation}
其中$\delta(C_u, C_v)$表示顶点$u, v$是否处于一个社区，即如果$C_v = C_u$，那么$\delta(c_u, c_v)$等于１；否则为０。
$w_{u,v}$表示边$(u,v)$的是否存在，即如果$(u,v)\in E$,那么$w_{u,v}=1$， 否则为0。
